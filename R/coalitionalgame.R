...
#' @param lex Logical parameter. If \code{lex = TRUE}, coalitions of claimants are ordered lexicographically. By default, \code{lex = FALSE}, and coalitions are ordered using their binary representations.
#' @param opt Logical parameter. If \code{opt = TRUE}, both the pessimist and optimistic associated coalitional games are given.
#' By default, \code{opt = FALSE}, and only the associated pessimistic coalitional game is computed.
...
#'
#'  Given a claims problem \eqn{(E,d)}, its associated pessimistic coalitional game is the game  \eqn{v_{pes}:2^N\rightarrow \mathbb{R}} assigning to each coalition \eqn{S\in 2^N},
#' \deqn{v_{pes}(S)=\max\{0,E-d(N\backslash S)\}.}
#'
#' Given a claims problem \eqn{(E,d)}, its associated optimistic coalitional game  is the game \eqn{v_{opt}:2^N\rightarrow \mathbb{R}}
#' assigning to each coalition \eqn{S\in 2^N},
#' \deqn{v_{opt}(S)=\min\{E,d(S)\}.}
#'
#' The optimistic and the pessimistic coalitional games are dual games, that is, for all \eqn{S\in 2^N},
#' \deqn{v_{opt}(S)=E-v_{pes}(N\backslash S).}
#'
#'
#' An efficient way to represent a nonempty coalition \eqn{S\in 2^N} is by identifying it with the binary sequence
#' \eqn{a_{n}a_{n-1}\dots a_{1},} where \eqn{a_i=1} if \eqn{i\in S} and \eqn{a_i=0} otherwise.
#' Therefore, each coalition \eqn{S} is represented by the number associated with its binary representation: \eqn{\sum_{i\in S}2^{i-1}}.
...
#' \deqn{v = [v(\{1\}),v(\{2\}),v(\{1,2\}),v(\{3\}),v(\{1,3\}),v(\{2,3\}),v(\{1,2,3\})].}
#' Alternatively, the coordinates can be ordered lexicographically:
#'  \deqn{v = [v(\{1\}),v(\{2\}),v(\{3\}),v(\{1,2\}),v(\{1,3\}),v(\{2,3\}),v(\{1,2,3\})].}
#'
...
#'
#' \eqn{v = [v(\{1\}),v(\{2\}),v(\{1,2\}),v(\{3\}),v(\{1,3\}),v(\{2,3\}),v(\{1,2,3\}),v(\{4\}),\dots}
#'
#' \eqn{\dots,v(\{1,4\}),v(\{2,4\}),v(\{1,2,4\}),v(\{3,4\}),v(\{1,3,4\}),v(\{2,3,4\}),v(\{1,2,3,4\})].}
#'
#' Alternatively, the coordinates can be ordered lexicographically:
#'
